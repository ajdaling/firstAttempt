...
//		}
	    ...
	    "bProcessing": true,
	    "bServerSide": true,
	    "sAjaxSource": "http://" + config.host + ":"+config.port+"/clusterURI.sjs",
...
function getClusters(table_cb){
	$.ajax({
	      type: "GET",
	      url: "http://"+config.host+":"+config.port+"/cluster.sjs",
	      data: reportData,
	      success: function(data){
//	    	  reportData.uriList = data.uriList;
	    	  console.log(data);
...
function createClusterFacetTable(clusters){
	...
	if(first){
		$(document).on("click","#clusterFacetTable > tr",function(d){
			...
			var docs = $(this).closest('tr').find('td:nth-child(2)').text();
			if($(this).hasClass('active')){
				delete reportData.uriList;
			} else{
				reportData.uriList = clusters.uriList;
				//queryFacets.docOrigin.push(orgn);
