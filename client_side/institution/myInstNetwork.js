...
// Add edges
	...
	cy.on('mouseover', 'node', function(evt){
		...
		hoverTimeout = setTimeout(function(){hoverElt.trigger('hover');},hoverDelay);
	})
	...
	//Add qtips for each node with its name
	...
	mainNode = cy.getElementById(thisCompName);
...
//		spacingFactor: 0.5,
	...
	cy.on('click', function(e){
		...
		if(sel!==cy && sel.isNode()){
			...
			showInfo(cy.nodes("[type='main']"));
			reset();
		}
	});

	//Populates table with top 10 institutions and researchers of main node
	var instTableData = [];
	var firstRow = [];
	firstRow.push(capitalize(mainNode.data('id')));
	firstRow.push(capitalize('Main'));
	firstRow.push('N/A');
	instTableData.push(firstRow);
	...
	instTable = $('#networkInfoTable').DataTable({
		"data" : instTableData,
		"order": [],
		"pageLength": 10,
		"lengthChange": false,
...
//					linkData = data.toLowerCase();
					if(row[1]=='Institution' || row[1]=='Main'){
		...
		if(layoutname == 'concentric1'){
			sublayout = nbhd.makeLayout({
				...
				concentric: function( node ){
					...
					if (node.data('type') == 'inst' || node.data('type') == 'main') {
						return 2;
					}
...
// Repopulates data in Organization Collaborators table
function showInfo(sel) {
	...
	firstRow.push('N/A');
	instTableData.push(firstRow);
	sel.openNeighborhood().nodes("[type!='res']").sort(function(a,b){
