...
//var req = d3.json("http://0.0.0.0:8081/query?phrase=neurology&numDocs=5000");
...
$.ajax({
 ...
 queue()
.defer("topics.csv", fnameTopics)
.defer(subtopics.csv, fnameSubTopics)
// .defer(d3.csv, fnameCounts)
.await(process);
var datapoints = req;

function process(error,topicInfo,subTopicInfo) {
function process(error, topicInfo,subTopicInfo) {
console.log("sub");
